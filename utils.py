...
def aggregate_dataset(df, grouping_field_list = None,  
                      array_field = None,
                     savepath = None):
    ...
    df = df.rename(columns={array_field: array_field + "_array"}).copy()
    ...
    concat_df.fillna(value= {k: 0 for k in dummy_col_list}, inplace=True)
    concat_df = concat_df.drop(columns = 'generic_drug_name_array')
    with open(savepath,'wb') as f:
        result = (concat_df, dummy_col_list)
